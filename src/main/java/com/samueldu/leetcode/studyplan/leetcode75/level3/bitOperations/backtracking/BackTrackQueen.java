...
public class BackTrackQueen {
     ...
     */
        private int backtrack(int row, Set<Integer> diagonals, Set<Integer> antiDiagonals, Set<Integer> cols) {
            ...
            // see below, row number increase by one only if a queen is placed on the board.
            if (row == size) {
                return 1;
            }
            ...
            for (int col = 0; col < size; col++) {
                ...
                // "Add" the queen to the board, pass current state in the path to the next recursive call
                cols.add(col);
                diagonals.add(currDiagonal);
                antiDiagonals.add(currAntiDiagonal);

                // Move on to the next row with the updated board state
                //note the row number increase by one only if a queen is placed in the previous 3 lines.
                solutions += backtrack(row + 1, diagonals, antiDiagonals, cols);

                // "Remove" the queen from the board since we have already
                // explored all valid paths using the above function call, clean up the state.
                cols.remove(col);
